		...
		$.get('/api/friends', function(data) {
			...
			function friendFinder() {
				...
				for (let i = 0; i < friends.length - 1; i++) {
					if (diff[i] === minimize && friends[i].name != 'user') {
						var friend = friends[i].name;
						var image = friends[i].photo;

						$('.modal-body').append(
							`Meet your new best friend! ${friend}
							${image}`
						);
						console.log('Meet your new best friend!', friend);
					}
				}
			}
			...
			function foeFinder() {
				...
				for (let i = 0; i < friends.length - 1; i++) {
					if (diff[i] === maximum) {
						var foe = friends[i].name;
						var image = friends[i].photo;

						$('.modal-body').prepend(
							`Your greatest enemy! ${foe}
							<img src='${image}'`
						);
						console.log('Revenge!!!', foe);
					}
				}
			}
